...
| `.agent/scripts/_plan_approach.py` | Plan-context extractor for `cross_model_review.sh` (`<plan.md>`): prints the plan's `## Approach` section verbatim, located with markdown-it-py (CommonMark) so fenced examples, indented/setext headings and thematic breaks are classified as a renderer would; exit 0 found / 1 no section / 2 markdown-it-py missing / 3 error. The caller tries the workspace `.venv` python3, then python3, and on 2 or 3 warns and omits the Plan Context block |
