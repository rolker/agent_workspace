...
| `.agent/scripts/review_progress.sh` | Review-loop helpers shared by review-code / triage-reviews / address-findings / plan-task / review-plan: `round` / `verdict` (convergence), `persist` (progress.md persistence behind `PROGRESS_PERSISTENCE_STRICT` / `--strict`; `--soft` = non-fatal), `sources` (local findings covering the head — recorded at it, or at an ancestor with only this issue's work-plan / roadmap changes since, via `_bookkeeping.sh` — plus GitHub comments at the head; only the newest covering review entry counts, older covering ones are listed as `superseded`, and entries that do not cover it as `stale` or, when git cannot check, `unverifiable` with a stderr warning), `findings` / `check` (latest review entry's open boxes), `plan-sha` (plan-commit SHA for the `**Plan**` field) |
