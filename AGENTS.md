...
| `.agent/scripts/cross_model_review.sh` | Cross-model adversarial review for a PR (`--pr <N>`) or local branch (`--branch [<ref>]`); `--agents a,b,c` runs several reviewers in parallel with one `AGENT=`/`FINDINGS_FILE=`/`EXIT=` triplet each (`--agent <x>` keeps the single-agent output), every agent bounded by a timeout (`AGENT_TIMEOUT`; gemini by its helper's `AGY_PRINT_TIMEOUT` plus an outer backstop above it) (ADR-0015, no tmux mode); `--no-progress` for skill worktrees; `--repo`, `--work-dir` for cross-repo use; the embedded diff excludes `.agent/work-plans/**` |
