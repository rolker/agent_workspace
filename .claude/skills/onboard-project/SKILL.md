...
Where `<repo-name>` is a directory under `project/`.
...
Find the repo under `project/<repo-name>`. Verify it exists and has a
recognizable project structure (build file, source directory, or manifest).

```bash
REPO_DIR=$(find project/<repo-name> -maxdepth 0 -type d 2>/dev/null | head -1)
```
...
Discover components/modules:
```bash
# Adapt to project type — examples:
find "$REPO_DIR" -name 'package.json' -o -name 'pyproject.toml' -o -name 'Cargo.toml' -o -name 'go.mod' \
...
| **License headers** | Source files have copyright/license headers | Report; too invasive for batch fix — always open issue |
...
1. Pre-commit config
2. CI workflow
3. Component labels (multi-component repos)
...
```bash
gh label create "component:<component_name>" --description "<component description>" \
...
- **Pre-commit config**: Copy template, replace protected branch placeholders
  ...
  for `*.py` files), uncomment the Python linting hooks (black, flake8).
  If it has JavaScript/TypeScript, include relevant linting hooks.

- **CI workflow**: Copy template, replace:
  - `PLACEHOLDER_DEFAULT_BRANCH` → repo's default branch
  - `PLACEHOLDER_BUILD_CMD` → appropriate build command (e.g., `make`,
    `npm run build`, `cargo build`, `go build ./...`)
  - `PLACEHOLDER_TEST_CMD` → appropriate test command (e.g., `make test`,
...
- **Component labels**: Create via `gh label create` (done in step 4, no
  file changes needed).
