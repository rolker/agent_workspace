...
| 6 | Port `ros2_colcon` adapter | #235, #237 | in progress | Pulled forward (target: host project11 with multi-distro). Phase 1 adapter core done (#235, PR #236). Phase 2 real-project smoke (#237): `setup` bootstraps a fresh hosting dir from the manifest URL; `p11-jazzy` registered and built on this machine. Next: layer worktrees (phase 3), second distro instance (phase 4, gated on the project-side `rolling` manifest branch) |
