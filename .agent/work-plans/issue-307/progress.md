...
## Integrated Review
**Status**: complete
**When**: 2026-09-22 09:19 -04:00
**By**: Claude Code Agent (claude-opus-5)

**PR**: #308 at `688d1e9`
**Sources**: 1 (Local Review round 4, PR mode @ `688d1e9`, approved — 0 must-fix, 3 suggestions; CI rollup @ `688d1e9`; the two Copilot reviews @ `8e9c0c8` and `688d1e9` are quota-exhausted comment-only placeholders carrying no finding — not sources)
**Cross-source confirmations**: 0
**CI**: all-pass — Lint (pre-commit), Validate Adapter Contract, Validate Documentation all green at `688d1e9`. The one `failure` check-run, `copilot-pull-request-reviewer`, is the quota exhaustion itself, not a repo gate.

### Findings
- [x] (minor/doc-accuracy, Local Review @ `688d1e9`) "One path is the exception" overstates singularity: a *passing* gate writes no merge entry either (`merge_pr.sh:889-890` prints the approval and calls no `_gate_record`), so a later failure — CI (`:1106-1132`), mergeability (`:1149-1167`), or `gh pr merge` (`:1205-1208`) — exits 1 with no `## Merge (report-only)` / `## Merge (unreviewed)` entry and therefore no `checkpoint:merge-refused` re-route (`dispatch_phase.sh:564-565`). Introduced by this PR at 5a8b71e. Fix: replace "One path is the exception" with a statement that the re-route depends on a merge entry existing, and name both entry-less paths (`--enforce` refusal on a workspace PR; a passing gate whose merge then fails) — `.claude/skills/run-issue/SKILL.md:271`
- [x] (minor/doc-accuracy, Local Review @ `688d1e9`) Step 11's sibling caveat is now less precise than the step-10 text it is supposed to mirror: it says "an `--enforce` refusal writes no entry and exits 1" without the `workspace` qualifier (`--enforce` on a project PR falls through to the report-only branch, `merge_pr.sh:896,904-908`), and its sentence still asserts that the `next` call routes to `checkpoint:merge-refused` for every non-merged outcome including that one. Fix: add the `workspace` qualifier and scope the re-route claim to runs that recorded a merge entry — `.claude/skills/run-issue/SKILL.md:403-407`
- [x] (minor/doc-accuracy, Local Review @ `688d1e9`) The "who writes what" table row states the gate writes `## Merge (report-only)` / `## Merge (unreviewed)` "when a precondition is unmet", with no note that `--enforce` on a workspace PR writes nothing — the same unqualified claim the step-10 fix corrects, now the last copy left. Fix: qualify the row (e.g. "except `--enforce` on a workspace PR, which refuses with no entry") — `.agent/knowledge/review_loop_lifecycle.md:34`

All three were verified against source at this head, not taken on the local reviewer's word: `merge_pr.sh:889-910` (gate pass records nothing; `--enforce` + `WORKTREE_TYPE == workspace` exits 1 before any `_gate_record`), `merge_pr.sh:1101-1208` (every post-gate failure path is a bare `exit 1`), and `dispatch_phase.sh:564-565` (only a `Merge (report-only)` / `Merge (unreviewed)` base emits `checkpoint:merge-refused`). None is a runtime defect — all three are prose in the loop's own operating docs — but finding 1 is text this PR itself added, and 2 and 3 are the same claim in its two sibling locations, so the three are one coherent prose edit, not three tasks.

The prior `## Integrated Review` at `8e9c0c8` is an earlier round: both of its findings are closed (the step-10 caveat in 5a8b71e, the helper bug deferred to follow-up #309) and re-verified here; nothing re-opens them. The known `sources` stale-correlation bug (#309) did not bite this round — the helper returned all three local findings at `688d1e9`.
