...
- [x] (suggestion) Stub isolation is scoped to `test_must_be_sourced` only; hoisting `make_offline_stubs` + the PATH override to suite level would make the offline guarantee structural, since this leak class already bit once — `.agent/scripts/tests/test_worktree_enter_stderr.sh:180`
