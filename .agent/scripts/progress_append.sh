...
# Prompt-free progress.md entry appender + committer.
# Ported from ros2_agent_workspace's .agent/scripts/progress_append.sh
# (issue #269 PR A) — see docs/decisions/0013-progress-md-entry-type-vocabulary.md.
#
# Usage:
...
#
# Scope discipline (what makes this script allowlist-safe):
...
#     is the job of check-commit-identity.py (pre-commit) and any CI-side
#     author check on agent-convention branches.
#
...
# (which a prompt-free caller would otherwise read as a real commit failure).
...
# silently overridden by a stale/human identity. Setting the four vars here
# makes the override authoritative; the `-c` flags remain as a fallback when
# GIT_* are unset.
