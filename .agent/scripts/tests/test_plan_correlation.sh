...
# --ref: the plan commit on another ref, resolved without touching the checkout
git -C "$REPO" checkout -q -b feature/issue-7-rev2
printf '# Plan rev 2\n' > "$REPO/.agent/work-plans/issue-7/plan.md"
git -C "$REPO" add -A && git -C "$REPO" -c user.name=t -c user.email=t@t commit -q -m "plan rev 2"
REV2_SHA=$(git -C "$REPO" rev-parse --short HEAD)
git -C "$REPO" checkout -q feature/issue-7
out=$(cd "$REPO" && "$RP" plan-sha --plan .agent/work-plans/issue-7/plan.md --ref feature/issue-7-rev2); rc=$?
[[ "$rc" -eq 0 && "$out" == "$REV2_SHA" ]] && pass "plan-sha --ref: the plan commit on the named ref ($REV2_SHA), current checkout untouched" || fail "plan-sha --ref (rc=$rc out=$out want=$REV2_SHA)"
out=$(cd "$REPO" && "$RP" plan-sha --plan .agent/work-plans/issue-7/plan.md --ref feature/issue-7); rc=$?
[[ "$rc" -eq 0 && "$out" == "$PLAN_SHA" ]] && pass "plan-sha --ref on the original branch still gives the plan commit, not that branch's head" || fail "plan-sha --ref original (out=$out)"
(cd "$REPO" && "$RP" plan-sha --plan .agent/work-plans/issue-7/plan.md --ref no-such-ref) >/dev/null 2>&1; rc=$?
[[ "$rc" -eq 2 ]] && pass "plan-sha --ref: an unresolvable ref is rc 2" || fail "plan-sha --ref bad ref (rc=$rc)"
(cd "$REPO" && "$RP" plan-sha --plan .agent/work-plans/issue-7/nope.md --ref feature/issue-7) >/dev/null 2>&1; rc=$?
[[ "$rc" -eq 2 ]] && pass "plan-sha --ref: a path absent at the ref is rc 2" || fail "plan-sha --ref missing path (rc=$rc)"
...
out=$(cd "$MIS" && printf '%s\n' "$ENTRY_PR" | WORKTREE_ISSUE=99 "$RP" persist --issue 7 --strict --soft 2>/dev/null); rc=$?
if [[ "$rc" -eq 0 && "$out" == "Progress persistence failed: refusing to resolve work-plans dir for issue #7 outside its worktree. (exit 4) — the report above is unaffected" ]] \
    && ! grep -qs '^## Plan Review$' "$MIS/.agent/work-plans/issue-7/progress.md"; then
    pass "persist --soft: a resolver refusal becomes ONE stdout notice (prefix stripped) with exit 0 and nothing written"
else
