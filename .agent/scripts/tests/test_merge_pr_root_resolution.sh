...
# caller set (which, like /tmp, is outside any git work tree — what
# test_resolution_outside_repo below depends on). Each test's
# setup/teardown still carves and drops its own TMPDIR_BASE under it; the
# trap is the backstop for an abort.
SANDBOX="$(mktemp -d)"
