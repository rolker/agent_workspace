...
Ported from ros2_agent_workspace's test_progress_read.py (issue #269 PR A),
extended with cases for this workspace's three new entry types (Checkpoint,
Merge (report-only), Merge (unreviewed) — see
...
or via the .agent/scripts/tests/test_progress_read.sh wrapper, which
run_script_tests.sh discovers.
"""
...
# A separate fixture with an actual `## External Review` heading — the GOLDEN
# fixture has none, so predecessor recognition needs its own fixture.
PREDECESSOR = """\
---
issue: 460
---
...
# This workspace's three new entry types (issue #269 PR A), not in the fork's
# ADR-0013.
NEW_TYPES = """\
---
issue: 269
...
**Resolver-hit**: resolved correctly
**Decision summary URL**: https://github.com/rolker/agent_workspace/pull/300#issuecomment-1

## Merge (report-only)
**Status**: complete
**When**: 2026-09-21 11:00 -04:00
**By**: merge_pr.sh (via progress_append.sh)

**PR**: #301 at `deadbee1`
**Conditions failed**: decision-summary comment missing
...
**PR**: #302 at `deadbee2`
**Reason**: --force-unreviewed bypass
"""
...
class TestNewEntryTypes(unittest.TestCase):
    """This workspace's three new ADR-0013 entry types (issue #269 PR A),
    not present in the fork's ADR-0013: Checkpoint, Merge (report-only),
    Merge (unreviewed)."""

    def setUp(self):
        self.result = parse_progress(NEW_TYPES, path="new_types.md")
        self.entries = _by_type(self.result)

    def test_all_three_parsed_and_recognized(self):
        self.assertEqual(len(self.result["entries"]), 3)
        for t in ("Checkpoint", "Merge (report-only)", "Merge (unreviewed)"):
            self.assertIn(t, self.entries)
            self.assertTrue(self.entries[t]["recognized"], t)

    def test_checkpoint_has_no_generic_correlation(self):
        # ADR-0013: Checkpoint has no canonical correlation key; consumers
        # check its required fields directly (test_checkpoint_269.sh).
        self.assertIsNone(self.entries["Checkpoint"]["correlation"])

    def test_merge_report_only_correlates_by_pr_sha(self):
        self.assertEqual(
            self.entries["Merge (report-only)"]["correlation"],
            {"kind": "pr", "pr": 301, "sha": "deadbee1"},
        )

    def test_merge_unreviewed_correlates_by_pr_sha(self):
        self.assertEqual(
            self.entries["Merge (unreviewed)"]["correlation"],
            {"kind": "pr", "pr": 302, "sha": "deadbee2"},
        )

    def test_merge_types_parenthetical_not_stripped(self):
        # "Merge (report-only)" / "Merge (unreviewed)" are canonical AS-IS,
        # same as "Local Review (Pre-Push)" — the parenthetical must not be
        # stripped down to a bare "Merge" base type.
        self.assertEqual(self.entries["Merge (report-only)"]["base_type"], "Merge (report-only)")
        self.assertEqual(self.entries["Merge (unreviewed)"]["base_type"], "Merge (unreviewed)")

    def test_cli_type_filter_for_checkpoint(self):
        data = _run_cli(NEW_TYPES, "--type", "Checkpoint")
        self.assertEqual([e["type"] for e in data["entries"]], ["Checkpoint"])
