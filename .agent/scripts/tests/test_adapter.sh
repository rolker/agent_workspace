...
EOF
    sed -i'' "s/\${STUB_NAME}/$name/" "$sb/bin/$name"
    chmod +x "$sb/bin/$name"
    echo "$sb/bin/$name"
}
...
test_default_project_type() {
    ...
    local sb out
    sb="$(make_sandbox)"
    out="$("$sb/.agent/scripts/adapter" project_root)"
...
test_from_flag_accepted() {
    ...
    local sb out
    sb="$(make_sandbox)"
    out="$("$sb/.agent/scripts/adapter" --from /nonexistent project_root)"
...
test_build_runs_in_project_root() {
    ...
    out="$("$sb/.agent/scripts/adapter" build --flag1)"
...
test_test_runs_in_project_root() {
    ...
    out="$("$sb/.agent/scripts/adapter" test)"
...
test_install_runs_when_set() {
    ...
    out="$("$sb/.agent/scripts/adapter" install)"
...
test_repos_format() {
    ...
    sb="$(make_sandbox)"
    make_git_repo "$sb/project" "git@github.com:owner/repo.git"
    out="$("$sb/.agent/scripts/adapter" repos)"
...
test_scope_for_pr_ssh() {
    ...
    out="$("$sb/.agent/scripts/adapter" scope_for_pr "$sb/repo_ssh")"
...
test_scope_for_pr_https() {
    ...
    out="$("$sb/.agent/scripts/adapter" scope_for_pr "$sb/repo_https")"
...
test_scope_for_pr_walks_up() {
    ...
    out="$("$sb/.agent/scripts/adapter" scope_for_pr "$sb/repo_walk/deep/sub/dir")"
...
EOF
    ...
    out="$("$sb/.agent/scripts/adapter" setup --answer 42)"
...
EOF
    out="$(python3 "$sb/.agent/scripts/sync_project.py" --dry-run)"
...
test_build_shim_chain() {
    ...
    make_git_repo "$sb/project" "git@github.com:owner/repo.git"
    echo "BUILD_CMD=\"$stub\"" > "$sb/.agent/project_config.sh"
    out="$("$sb/.agent/scripts/build.sh")"
    assert_contains "stub reached via shim" "STUB_shimbuild_RAN_IN:" "$out"
}
